...
截至当前快照（2026-02-22），项目结论口径为：**严格审批未通过（REJECTED / WEAK_SIGNAL）**，统一评审为 **L1**；当前仅可表述为“观察到时序相关”，尚不满足因果识别闸门。
...
| 乌克兰通话丑闻（弹劾调查序幕） | 2019-07-25 | **+55天** | 海军官方确认UFO视频真实性 | 2019-09-18 |
| COVID危机+特朗普遭批（国家紧急状态） | 2020-03-13 | **+45天** | 五角大楼正式解密UFO视频 | 2020-04-27 |
...
| 爱泼斯坦文件争议（DOJ版本不完整） | 2025-12-23 | **+58天** | 特朗普下令公开所有外星人/UFO文件 | 2026-02-19 |

已剔除案例（方向性不符）：
- Storm Area 51（2019-06-27）早于乌克兰通话危机（2019-07-25），属于负间隔事件，不纳入“危机后UFO上升”主证据链。
...
> 注意：`causal_analyzer.py` 现增加 `placebo_treatment_not_dominant` 审批闸门：若伪冲击在审批窗口内出现显著且同量级效应，将阻断“因果信号”结论放行。
...
> 注意：`panel_pipeline.py` 在传入 `--shock-catalog-file` 时，也会默认执行同样的锁一致性 fail-fast 校验（支持 `--shock-lock-file` / `--skip-shock-lock-check`）。
...
> ⚠️ **方法说明**：以下数字来自人工筛选的正向配对样本（events_v2），存在选择偏差，仅供描述性参考，不构成统计推断依据。面板分析结论以 `data/causal_report.json` 为准。

- **平均间隔**：人工筛选的10个案例中，政治危机到UFO新闻的平均间隔约 **41天**
